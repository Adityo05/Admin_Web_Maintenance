        ...
        <form action="{{ route('karyawan.store') }}" method="POST" style="padding: 24px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                <div class="form-group">
                    ...
                    <select id="jabatan" name="jabatan" class="form-input" required onchange="toggleMesinSelection(this.value)">
                        <option value="">Pilih Jabatan</option>
                        <option value="Teknisi">Teknisi</option>
                        <option value="KASIE Teknisi">KASIE Teknisi</option>
                    </select>
                    @error('jabatan')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
            ...
            <div class="form-group" id="mesinSelectionContainer">
                ...
                <div style="max-height: 300px; overflow-y: auto; border: 1px solid #e0e0e0; border-radius: 8px; padding: 12px;">
                    ...
                    <label style="display: flex; align-items: center; gap: 8px; padding: 8px; cursor: pointer; border-radius: 4px; transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#f5f5f5'" onmouseout="this.style.backgroundColor='transparent'">
                        <input type="checkbox" name="assets[]" value="{{ $asset->id }}" class="asset-checkbox" style="width: 18px; height: 18px; cursor: pointer;">
            ...
            <script>
            function toggleMesinSelection(jabatan) {
                const container = document.getElementById('mesinSelectionContainer');
                const checkboxes = document.querySelectorAll('.asset-checkbox');
                
                if (jabatan === 'KASIE Teknisi') {
                    container.style.opacity = '0.5';
                    container.style.pointerEvents = 'none';
                    // Uncheck all checkboxes for KASIE Teknisi
